## Quick preview v2 — loads all enemy models side by side. Run with F6. Delete when done.
extends Node3D
...
func _ready() -> void:
	...
	back_light.rotation_degrees = Vector3(-30, -135, 0)
	...
	# Spawn each enemy model
	var spacing := 3.5
	var total_width := (ENEMIES.size() - 1) * spacing
	var start_x := -total_width / 2.0
	var i := 0

	for enemy_name in ENEMIES:
		var path: String = ENEMIES[enemy_name]
		var scene = load(path)
		if not scene:
			print("ERROR: Failed to load %s from %s" % [enemy_name, path])
			i += 1
			continue
		...
		# Measure raw bounds before any scaling
		var raw_bounds := _get_bounds(model)
		print("%s — raw bounds: %s (height: %.2f)" % [enemy_name, raw_bounds, raw_bounds.size.y])

		# Scale ALL models to the same uniform height (multiply to preserve FBX transforms)
		if raw_bounds.size.y > 0.01:
			var s := UNIFORM_HEIGHT / raw_bounds.size.y
			s = clampf(s, 0.001, 100.0)
			model.scale *= s
			print("  -> scale factor: %.4f, final scale: %s" % [s, model.scale])

		# Foot alignment (recompute after scaling)
		model.position.y = 0.0
		var scaled_bounds := _get_bounds(model)
		if scaled_bounds.size.y > 0.01:
			model.position.y = -scaled_bounds.position.y

		# Attach helmet to Ely — 1.62 is the head Y in raw model coords
		if enemy_name == "Hallucination":
			var helmet := _build_robot_helmet()
			var ms: float = model.scale.x
			var size_ratio := UNIFORM_HEIGHT / 2.0  # Helmet 1.25x bigger to match bigger model
			var inv_s := size_ratio / ms
			helmet.scale = Vector3(inv_s, inv_s, inv_s)
			# Use raw model-local Y=1.62 — parent scale moves it to the correct world position
			helmet.position = Vector3(0, 1.62, 0)
		...
		label.position = Vector3(root.position.x, -0.3, 0)
		add_child(label)
	...
	cam.position = Vector3(0, 1.8, 10.0)
	cam.look_at(Vector3(0, 1.2, 0))
	cam.current = true
	cam.fov = 50.0
	add_child(cam)

	print("Enemy preview ready! %d models loaded." % _models.size())
...
func _get_bounds(node: Node) -> AABB:
	...
	_collect_mesh_points(node, Transform3D.IDENTITY, points)
	if points.is_empty():
		return AABB()
...
func _build_robot_helmet() -> Node3D:
	...
	armor_mat.albedo_color = Color(0.20, 0.14, 0.16)
	...
	armor_dark.albedo_color = Color(0.10, 0.07, 0.09)
